#!/usr/bin/env python3
"""
scripts/compute_matrices_flexible.py
---------------------

Computes and visualizes adjacency matrices for feature similarity
and (optionally) LightGlue match counts.

This script:
1. Loads features and image paths.
2. Initializes LightGlue (if not skipped).
3. Computes the full (N x N) feature similarity matrix (Cosine Similarity).
4. IF NOT SKIPPED: Computes the FULL (N x N) LightGlue match count matrix.
5. Saves and visualizes the computed matrices.

Usage:
  # Compute BOTH matrices (very slow)
  python scripts/compute_matrices_flexible.py --features_path tmp/landmarks_features.npy \
                                              --paths_txt tmp/landmarks_paths.txt

  # Compute ONLY the Feature Similarity matrix (fast)
  python scripts/compute_matrices_flexible.py --features_path tmp/landmarks_features.npy \
                                              --paths_txt tmp/landmarks_paths.txt \
                                              --skip_lightglue
"""
...
def parse_args() -> argparse.Namespace:
    ...
    # --- MODIFICATION: Added skip flag ---
    parser.add_argument(
        "--skip_lightglue",
        action="store_true",
        help="Only compute the feature similarity matrix and skip the slow LightGlue matrix."
    )
    # --- End Modification ---
...
def main():
    ...
    # --- MODIFICATION: Wrap Step 3 in a conditional block ---
    if not args.skip_lightglue:
        ...
        print("\n[3/4] Skipping LightGlue matrix computation as requested.")
    # --- End Modification ---
    ...
    # --- MODIFICATION: Wrap LightGlue plotting in a conditional block ---
    if not args.skip_lightglue:
        if args.out_lg_plot:
            # We need to load lg_matrix if it was computed
            if 'lg_matrix' not in locals():
                # This case might happen if file exists but we skipped compute
                # For safety, let's just assume it was computed in this block
                lg_matrix = np.load(args.out_lg_matrix) 
        ...
        if args.out_lg_plot:
             print(f"   Skipping LightGlue plot: {args.out_lg_plot}")
    # --- End Modification ---
